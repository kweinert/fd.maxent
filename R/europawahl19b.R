europawahl19b <- function(
	...
	mep <- mep_make(
		nvar=length(prob), 
		ncons=length(parties) + (length(gender)*length(edu)) + (1*nrow(tab.gender.votes)) + (3*nrow(tab.edu.votes)) + 1, # length(gender)
	...
	for (x in rownames(tab.gender.votes)) {
		...
		# browser()
		# prct <- tab.gender.votes[x, "female"]
		# mep <- mep_set_constraint(mep, j=j+1, 
			# xt=c(rep(-1+prct, times=length(edu)), rep(prct, times=(length(voters)-1)*length(edu))), 
			# indices=c(index(prob, vote=x, mf="female"), index(prob, vote=setdiff(voters,x), mf="female")), 
			# rhs=0
		# )
		j <- j + 1
	}
